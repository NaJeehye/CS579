...
:- set_prolog_stack(global, limit(100_000_000_000)).
:- set_prolog_stack(trail,  limit(20_000_000_000)).
:- set_prolog_stack(local,  limit(2_000_000_000)).
...
curt:- 
   format('Hello! If have any question, plz tell me!',[]),
   curtUpload,
   curtTalk(run).
...
curtUpload:-
    open('TMP.txt', read, Str),
    readStr(Str,StrList),
    close(Str),
    curtUploadTXT(StrList).
...
curtUploadTXT([Input|L]):-
    curtUpdate(Input,_,run), 
    curtUploadTXT(L).
...
consistent([Old|_],New,Model):-
   ...
   DomainSize=15,
   callTPandMB(not(and(and(BK,Old),New)),and(and(BK,Old),New),DomainSize,Proof,Model,_),
   \+ Proof=proof, Model=model([_|_],_).

consistent([],New,Model):-
   backgroundKnowledge(New,BK),
   DomainSize=15,
   callTPandMB(not(and(BK,New)),and(BK,New),DomainSize,Proof,Model,_),
   \+ Proof=proof, Model=model([_|_],_).
...
informative([Old|_],New):-
   backgroundKnowledge(and(Old,New),BK),
   DomainSize=15,
   callTPandMB(not(and(and(BK,Old),not(New))),and(and(BK,Old),not(New)),DomainSize,Proof,Model,_),
   \+ Proof=proof, Model=model([_|_],_).  

informative([],New):-
   backgroundKnowledge(New,BK),
   DomainSize=15,
   callTPandMB(not(and(BK,not(New))),and(BK,not(New)),DomainSize,Proof,Model,_),
   \+ Proof=proof, Model=model([_|_],_).
